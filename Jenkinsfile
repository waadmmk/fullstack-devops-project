pipeline {
  ...
  stages {
    ...
    stage('SonarQube') {
      ...
      post {
        success { notifyStage('Backend Sonar', 'SUCCESS') }
        failure { notifyStage('Backend Sonar', 'FAILURE') }
      }
    }
    ...
    stage('Frontend Sonar') {
      ...
      post {
        success { notifyStage('Frontend - Sonar', 'SUCCESS') }
        failure { notifyStage('Frontend - Sonar', 'FAILURE') }
      }
    }
  }
}
...
stage('Nexus Upload') {
  parallel {
    stage('Backend-to-Nexus') {
      ...
      post {
        success { notifyStage('Backend-to-Nexus', 'SUCCESS') }
        failure { notifyStage('Backend-to-Nexus', 'FAILURE') }
      }
    }

    stage('Frontend-to-Nexus') {
      steps {
        dir('frontend') {
          sh "tar -czf frontend-0.0.1-${BUILD_NUMBER}.tgz dist"
          nexusArtifactUploader artifacts: [
            [
              artifactId: 'frontend',
              classifier: '',
              file: "frontend-0.0.1-${BUILD_NUMBER}.tgz",
              type: 'tgz'
            ]
          ],
          credentialsId: 'nexus',
      ...
      post {
        success { notifyStage('Frontend-to-Nexus', 'SUCCESS') }
        failure { notifyStage('Frontend-to-Nexus', 'FAILURE') }
      }
    }
  }
}
     stage('Docker Build & Push (Ansible)') {
       steps {
  ...
  steps {
    dir('ansible') {
      sh 'ansible-playbook -i inventory.ini k8s-playbook.yaml'
    }
  }
                  post {
...
// ===== Function to send email using Email Extension Plugin =====
